...
if pnet.parallel.main(__name__):
    ...
    if 0:
        ...
        ##second-layer parts
        partsRegion = np.load('/var/tmp/poolExtractedFeatureRot%d.npy' % RotNo)
        numTrueParts = 50
        ...
        allPartsLayer = [[pnet.PartsLayer(numSecondLayerParts,(1,1),
                            ...
                            max_samples=10000, 
                            min_prob = 0.005))] 
                            for i in range(numTrueParts)]
        allPartsLayerImg = np.zeros((numTrueParts,numSecondLayerParts,secondLayerShape,secondLayerShape))
        allPartsLayerImgNumber = np.zeros((numTrueParts,numSecondLayerParts))
        zeroParts = 0
        ...
        for i in range(numTrueParts):
            ...
            for j in range(extractedFeaturePart.shape[0]):
                if(extractedFeaturePart[j,0,0,0]!=-1):
                    ...
                    allPartsLayerImg[i,partIndex]+=allTrainData[imgRegion[i * 16 + RotNo][j]]
        ...
        settings['vmin'] = 0
        settings['vmax'] = 1
        plotData = np.ones(((2 + secondLayerShape)*80+2,(2+secondLayerShape)*(numSecondLayerParts + 1)+2))*0.8
        visualShiftParts = RotNo
        if 0:
            ...
            for i in range(numSecondLayerParts + 1):
                for j in range(numTrueParts):
                    if i == 0:
                        plotData[5 + j * (2 + secondLayerShape):5+firstLayerShape + j * (2 + secondLayerShape), 5 + i * (2 + secondLayerShape): 5+firstLayerShape + i * (2 + secondLayerShape)] = partsPlot[16 * j+visualShiftParts]
                    else:

...
if pnet.parallel.main(__name__):
    ...
    if 1:
        classificationLayers = [
                            pnet.PoolingLayer(shape = (5,5),strides = (5,5)),
            ...
            for m in range(29 - secondLayerShape):
                for n in range(29 - secondLayerShape):
                    if(secondLevelCurTestXCenter[i,m,n]!=-1):
                        ...
                        extractedFeaturePart = extract(np.array(secondLevelCurTestX[i,m,n][np.newaxis,:],dtype = np.uint8),secondLayerPartsLayer)[0]
                        thirdLevelCurTestX[i,m,n] = extractedFeaturePart        
        end = time.time()
        print(end-afterPool)

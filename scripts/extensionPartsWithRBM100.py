...
if pnet.parallel.main(__name__):
    ...
    #np.save('weights100Hidden.npy',weights)
    weights = np.load('weights100Hidden.npy')
    weights = weights.reshape(4,4,500,100)
    ...
    for p in range(4):
        for q in range(4):
            ...
            X = np.array(data.reshape(500,100),dtype=np.double)
            ...
            for k in np.where(np.max(location1,axis=0)!=0)[0]:
                if kmeans[k] == skipIndex:
                    ...
                    numPooling = (distanceArray.shape[0] + 1)//20
    #                 print(numPooling)
